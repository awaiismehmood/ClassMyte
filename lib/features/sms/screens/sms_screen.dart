...
import 'package:classmyte/core/providers/device_provider.dart';
...
class _NewMessageScreenState extends ConsumerState<NewMessageScreen> {
                    ...
                    Expanded(
                      child: SingleChildScrollView(
                        ...
                        child: Column(
                          ...
                          children: [
                            _buildRemoteProcessLock(context, ref),
                            ...
                            ValueListenableBuilder<bool>(
                              ...
                              builder: (context, isSending, _) => CustomButton(
                                ...
                                onPressed: isSending
                                    ...
                                    : () async {
                                        final progress = ref.read(smsProgressProvider);
                                        final localDevice = await ref.read(deviceInfoProvider.future);
                                        
                                        if (progress.status == 'sending' && progress.deviceId != localDevice.id) {
                                          CustomSnackBar.showWarning(context, 'A process is already running on another device.');
                                          return;
                                        }
  ...
  Widget _buildRemoteProcessLock(BuildContext context, WidgetRef ref) {
    final progress = ref.watch(smsProgressProvider);
    final isDark = Theme.of(context).brightness == Brightness.dark;
    final deviceFuture = ref.watch(deviceInfoProvider.future);
