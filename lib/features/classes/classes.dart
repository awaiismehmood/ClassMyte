...
class ClassScreen extends ConsumerWidget {
        ...
        children: [
          CustomHeader(
            ...
            rightActions: [
              _buildCircleAction(
                icon: Icons.add,
                onTap: () => showAddContactDialog(context, () => ref.invalidate(studentDataProvider)),
              ),
            ],
          ),
          Expanded(
                  ...
                  child: Column(
                    children: [
                      Padding(
                        ...
                        child: _buildBadge('Total Classes: ${allClasses.length}', Colors.red),
                      ),
                      Expanded(
  ...
  Widget _buildBadge(String text, Color color) {
    return Container(
      ...
      child: Text(text, style: GoogleFonts.outfit(fontSize: 14, fontWeight: FontWeight.bold, color: color)),
    );
  }
  ...
  Widget _buildClassCard(BuildContext context, WidgetRef ref, String className, int count, List<String> allClasses, List<Map<String, String>> allStudents) {
    return Card(
      ...
      child: ListTile(
        ...
        title: Text('Class $className', style: GoogleFonts.outfit(fontWeight: FontWeight.bold, fontSize: 18, color: AppColors.primary)),
        subtitle: Text('Total students: $count', style: GoogleFonts.outfit(color: AppColors.textSecondary)),
        trailing: Row(
