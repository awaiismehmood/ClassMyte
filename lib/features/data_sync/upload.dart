      ...
      try {
        ...
        for (var row in sheet.rows.skip(1)) {
          try {
            ...
            if (row.length < 7) {
              print(
                  'Skipping invalid or incomplete row: ${row.map((cell) => cell?.value).toList()}');
              continue;
            }
            ...
            print(
                'Error processing row: ${row.map((cell) => cell?.value).toList()}');
